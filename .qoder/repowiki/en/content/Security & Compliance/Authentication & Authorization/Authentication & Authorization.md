...
## Passkey Smart Wallet Authentication (Protocol 21 / secp256r1)

**Added in issue #974.**

Referenced files:
- [passkey.ts](file://src/lib/wallet/passkey.ts)
- [smartWallet.ts](file://src/lib/wallet/smartWallet.ts)
- [SmartWalletPanel.tsx](file://src/components/dashboard/SmartWalletPanel.tsx)
- [SECURITY.md](file://SECURITY.md)

### Overview

Protocol 21 added secp256r1 (P-256) signature verification to the Soroban host.  This enables **passkey smart wallets** — Soroban contract accounts (C-addresses) whose `__check_auth` function verifies WebAuthn / P-256 signatures instead of Ed25519.

Unlike classic G-address accounts:
- The contract account has no Ed25519 keypair. All authorization flows through `__check_auth`.
- The P-256 private key is generated by and permanently bound to a platform authenticator (Touch ID, Windows Hello, FIDO2 key). It never leaves the device.
- The contract pays fees via a fee-sponsor relayer; the user does not need to hold XLM.

### Authentication Flow

```
1. User creates a passkey → navigator.credentials.create({ pubKeyCredParams: [{ alg: -7 }] })
   → Authenticator generates P-256 keypair, returns credentialId + compressed public key
   → Public key is registered in the smart wallet contract (deploy or attach step)

2. User invokes a contract operation → dashboard builds a SorobanAuthorizationEntry
   → challenge = SHA-256(network_passphrase || auth_entry_xdr)
   → navigator.credentials.get({ challenge }) prompts the user
   → Authenticator signs challenge → returns authenticatorData + clientDataJSON + DER signature

3. Dashboard builds a credential ScMap:
   { auth_data: bytes, client_data: bytes, signature: bytes, pk: bytes }
   → Encoded as Base64 XDR and attached to the auth entry

4. Auth entry + unsigned transaction XDR sent to fee-sponsor relayer
   → Relayer wraps into fee-bump, broadcasts to Soroban RPC
   → Contract __check_auth verifies the P-256 signature on-chain
```
...
|---------|----------------|-------|
| Chrome 67+ | ✓ | Platform + security key |
| Edge 18+ | ✓ | Platform + security key |
| Safari 14+ | ✓ | Touch ID / Face ID |
| Firefox 60+ | ✓ | Security key only (no platform) |
| Other | ✗ | `isPasskeySupported()` returns false; error shown |

### Error Handling

All passkey errors are typed and handled explicitly:

| Error class | Trigger | User message |
|-------------|---------|--------------|
| `PasskeyNotSupportedError` | `PublicKeyCredential` absent | Browser not supported banner |
| `PasskeyCancelledError` | `NotAllowedError` from authenticator | "Prompt was cancelled, try again" |
| `PasskeyCredentialNotFoundError` | Credential absent / `SecurityError` | "Credential not found on this device" |
| `PasskeyTimeoutError` | `TimeoutError` from authenticator | "Authenticator did not respond, try again" |

### Migration Notes

Existing G-address users are unaffected. Both account types can coexist in the same dashboard session. Smart wallet accounts appear in the **Passkey Smart Wallet** tab; classic accounts continue to appear in the **Account** tab as before.

To migrate from a classic account to a smart wallet:
1. Deploy a passkey smart wallet contract on testnet/mainnet.
2. Register your passkey's compressed public key in the contract.
3. Connect the C-address in the Smart Wallet panel.
4. Fund the contract via a SAC transfer or direct XLM transfer from your G-address.

### Security Posture

See the **Passkey Smart Wallet Threat Model** section in [SECURITY.md](file://SECURITY.md) for a full threat analysis including origin binding, relayer trust, sign-count replay protection, and recovery recommendations.
