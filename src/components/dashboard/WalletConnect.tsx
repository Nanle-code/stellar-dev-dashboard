...
function SecurityBadge({ posture }: { posture: SecurityPosture }) {
  const color =
    posture.tier === 'high'
      ? 'var(--green)'
      : posture.tier === 'medium'
...
function SocialRecoveryPanel() {
  ...
  const [result, setResult] = useState<SocialRecoveryConfig | null>(null)
  ...
  return (
    ...
    >
      ...
      <textarea
        value={guardians}
        onChange={(e: React.ChangeEvent<HTMLTextAreaElement>) => setGuardians(e.target.value)}
...
export default function WalletConnect() {
  ...
  const connectCommon = async (type: string, publicKey: string, options: ConnectOptions = {}) => {
    ...
    try {
      ...
      refreshAudit({ action: 'account_load_error', status: 'error', details: message })
    } finally {
      setAccountLoading(false)
    }
  }

  const handleSoftwareConnect = async (walletId: string) => {
    setConnecting(true)
  ...
  const handleDisconnect = async () => {
    ...
    refreshAudit({
      ...
      status: 'info',
      details: `${walletType || 'wallet'} disconnected`,
    })
    disconnectWallet()
  ...
  return (
    <Card title="Connect Wallet" subtitle="Software, mobile, hardware, and WalletConnect v2">
      <div style={{ padding: '18px', display: 'flex', flexDirection: 'column', gap: '12px' }}>
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
            <label style={{ fontSize: '11px', color: 'var(--text-muted)' }} htmlFor="ledger-path-select">
              Ledger account (derivation path)
            </label>
            <select
              id="ledger-path-select"
              value={selectedDerivationPath}
              onChange={(e: React.ChangeEvent<HTMLSelectElement>) => setSelectedDerivationPath(e.target.value)}
              style={{
                background: 'var(--bg-card)',
                border: '1px solid var(--border)',
                borderRadius: 'var(--radius-sm)',
            ...
            >
              {LEDGER_DERIVATION_PATHS.map((p: { path: string; label: string }) => (
          ...
          <input
            ...
            onChange={(e: React.ChangeEvent<HTMLInputElement>) => setManualHardwareKey(e.target.value)}
            placeholder="G... public key for Trezor / Keystone (watch-only)"
          ...
          {HARDWARE_WALLET_DEVICES.map((device: { id: string; name: string; description: string; features: string[] }) => (
            <button
              key={device.id}
        ...
        <div>
          ...
          {showSocialRecovery && <SocialRecoveryPanel />}
        </div>
        ...
        <div style={{
          ...
          <input
            value={safetyInput}
            onChange={(e: React.ChangeEvent<HTMLInputElement>) => setSafetyInput(e.target.value)}
