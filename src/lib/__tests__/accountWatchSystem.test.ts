...
describe('AccountWatchSystem', () => {
  ...
  it('reports balance deltas and persists a per-network last-visit marker', async () => {
    const fetchAccount = vi.mocked(stellar.fetchAccount)
    fetchAccount.mockResolvedValueOnce({ balances: [{ asset_type: 'native', balance: '5' }] } as never)
      .mockResolvedValueOnce({ balances: [{ asset_type: 'native', balance: '8' }] } as never)
    const sys = new AccountWatchSystem()
    sys.addAccount(VALID)
    const first = await sys.refresh()
    expect(first.changes).toHaveLength(0)
    const second = await sys.refresh()
    expect(second.changes[0].balanceDeltas[0]).toMatchObject({ assetCode: 'XLM', balance: 3 })
    const seen = sys.markAllSeen()
    expect(seen).toBeGreaterThan(0)
    expect(sys.getLastVisit()).toBe(seen)
  })
})
