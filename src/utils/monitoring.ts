...
// ─── Health snapshot helpers (re-exported from monitoring.js) ─────────────────
// useMonitoring.js imports these directly from utils/monitoring (which resolves
// to this .ts file before the .js file due to Vite's extension priority).
...
function _readMemory(): MemoryInfo | null {
...
function _readNavigationTiming(): NavigationTiming | null {
...
/** Collect a lightweight synchronous health snapshot of the current browser environment. */
export function collectHealthSnapshot(): HealthSnapshot {
  return {
    ...
    memory: _readMemory(),
    navigation: _readNavigationTiming(),
  };
}

const _latencyHistory: Array<{ timestamp: string; latency: number }> = [];
const _MAX_LATENCY_POINTS = 96;

function _recordLatencySample(latency: number): void {
  if (!Number.isFinite(latency)) return;
  _latencyHistory.push({ timestamp: new Date().toISOString(), latency });
  const cutoff = Date.now() - 24 * 60 * 60 * 1000;
  while (_latencyHistory.length && Date.parse(_latencyHistory[0].timestamp) < cutoff) {
    _latencyHistory.shift();
  }
  while (_latencyHistory.length > _MAX_LATENCY_POINTS) {
    _latencyHistory.shift();
  }
}

/** Collect health snapshot including network probe results. */
export async function collectSystemHealthSnapshot(): Promise<SystemHealthSnapshot> {
  ...
  try {
    const { probeAllNetworks } = await import('../lib/stellar');
    networkHealth = await probeAllNetworks();

    const latencies = (networkHealth as Array<{ horizon?: { latency?: number } }>)
      .map((n) => n.horizon?.latency)
      .filter((l): l is number => typeof l === 'number' && Number.isFinite(l));

    if (latencies.length) {
      const avg = latencies.reduce((s, v) => s + v, 0) / latencies.length;
      _recordLatencySample(avg);
    }
  } catch {
    // network probe failed — continue without network data
  }
...
/** Compute an overall health score (0–100) from a snapshot. */
export function computeHealthScore(snapshot: HealthSnapshot | null): number {
  ...
  if (snapshot.navigation?.loadEventMs != null) {
...
type ErrorWatchHandler = (err: { kind: string; message: string; timestamp: string }) => void;

/** Subscribe to global runtime errors and unhandled rejections. Returns an unsubscribe fn. */
export function watchErrors(onError: ErrorWatchHandler): () => void {
  const ts = () => new Date().toISOString();

  function handleError(event: ErrorEvent) {
    onError({ kind: 'runtime-error', message: event.message || 'Unknown runtime error', timestamp: ts() });
  }

  function handleRejection(event: PromiseRejectionEvent) {
    onError({ kind: 'promise-rejection', message: String(event.reason ?? 'Unhandled rejection'), timestamp: ts() });
  }
